...
jest.mock("bcrypt", () => ({
	...
	hash: jest.fn().mockResolvedValue("Password123@"), // Set the return value for hash
}));
...
describe("CreateUserUseCase (Unit Tests)", () => {
	...
	beforeEach(() => {
		...
		mockPostgresUsersRepository.save = jest.fn().mockResolvedValue({
			...
			name: "user",
			email: "user@user.com",
			password: "Password123@", // Normally this would be a hashed password generated by bcrypt
		});
	...
	it("should save the user with the provided information", async () => {
		...
		const password = "Password123@"; // Alterado para incluir uma letra maiúscula
		...
		expect(mockPostgresUsersRepository.save).toHaveBeenCalledWith(
			expect.objectContaining({
				name,
				email,
				password: "Password123@", // Ensuring that the hashed password is passed to save
			})
		);
	});
	...
	it("should throw an error if the user already exists", async () => {
		...
		await expect(
			createUserUseCase.execute({
				name: "user",
				email: "user@user.com",
				password: "Password123@", // Alterado para incluir uma letra maiúscula
			})
		).rejects.toThrow("User already exists"); // Ajuste aqui para "User already exists"
	...
	it("should hash the password before saving", async () => {
		...
		expect(bcrypt.hash).toHaveBeenCalledWith(
			"Password123@", // A senha agora contém uma letra maiúscula
			expect.any(Number)
		);
	});
});
